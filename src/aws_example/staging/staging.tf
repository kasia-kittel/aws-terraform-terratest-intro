...
module "aws-ec2-frontend" {
  ...
  vpc-sg-ids = [module.aws-sg.private-ssh-sg-id, module.aws-lb.lb-tcp-sg-id]
...
module "aws-network" {
  ...
  availability-zone = data.aws_availability_zones.available.names[0] // this should be enought to have all instances in the smae AZ
}
...
module "aws-lb" {
  ...
  vpc-id = module.aws-network.main-vpc-id
  public-subnet-id = module.aws-network.public-subnet-id
  frontend-instance-id = module.aws-ec2-frontend.aws-ec2-id
  private-subnet-cidr = local.private-subnet-cidr
  public-subnet-cidr = local.public-subnet-cidr
}
...
output "lb-dns-name" {
  value = module.aws-lb.lb-dns-name
}
