terraform {
  backend "s3" {
    bucket         = "ee-tf-remote-state-kkittel"
    key            = "staging/terraform.tfstate"
    region         = "eu-west-2"
    dynamodb_table = "ee-tf-locks-kkittel"
    encrypt        = true
  }
}

provider "aws" {
  region = var.region
}

resource "aws_key_pair" "kasia" {
  key_name   = "kasia-key"
  public_key = "ssh-rsa  kasia@Katarzynas-MBP"
}

module "aws-ec2-bastion" {
  source = "../modules/aws-ec2-generic"
  aws-ec2-name  = "bastion"
  vpc_sg_id = [module.aws-network.public-ssh-sg-id]
  subnet_id = module.aws-network.public-subnet-id
  key_name = aws_key_pair.kasia.key_name
}

module "aws-ec2-ahost" {
  source = "../modules/aws-ec2-generic"
  aws-ec2-name  = "ahost"
  vpc_sg_id = [module.aws-network.private-ssh-sg-id]
  subnet_id = module.aws-network.private-subnet-id
  key_name = aws_key_pair.kasia.key_name 
}

module "aws-network" {
  source = "../modules/aws-network"
  main-vpc-cidr  = "10.10.0.0/16"
  public-subnet-cidr = "10.10.1.0/24"
  private-subnet-cidr = "10.10.2.0/24"
}

output "bastion_ip" {
  value = module.aws-ec2-bastion.instance_public_ip
}

output "host_private_ip" {
  value = module.aws-ec2-ahost.instance_private_ip
}